...
class DashboardController extends Controller
{
    ...
    public function sortOrders(Request $request, $criteria)
    {
        // Xử lý sắp xếp đơn hàng dựa trên tiêu chí $criteria
        // Ví dụ: sắp xếp theo ngày
        if ($criteria == 'date') {
            $orders = Order::where('user_id', auth()->id())->orderBy('created_at')->get();
        }
        
        // Trả về view hiển thị danh sách đơn hàng đã được sắp xếp
