...
class HomeController extends Controller
{
    ...
    function shop(Request $request)
    {
        ...
        $sortOption = $request->input('sort', Session::get('sort', ''));
        
        // Lưu giá trị sort vào session
        Session::put('sort', $sortOption);
    
        // Sắp xếp sản phẩm dựa trên giá trị $sortOption
        if ($sortOption === 'low_to_high') {
            $products = Product::orderByRaw("CAST(current_price AS DECIMAL(10, 2))")->get();
        } elseif ($sortOption === 'high_to_low') {
            $products = Product::orderByRaw("CAST(current_price AS DECIMAL(10, 2)) DESC")->get();
        } else {
            $products = Product::latest()->get();
        }
    
    
        return view('user.shop', compact('categories', 'products', 'brands', 'sortOption'));
    }

    function search_product(Request $request)
    {
        $category = $request->slug === 'all' ? null : Category::whereSlug($request->slug)->first();
        ...
        // If category is set to "all", retrieve all products regardless of category
        if ($category === null) {
        ...
        if ($sortOption === 'low_to_high') {
            $query->orderByRaw("CAST(current_price AS DECIMAL(10, 2))");
        } elseif ($sortOption === 'high_to_low') {
    ...
    function product_by_category($slug, Request $request): View
    {
    ...
    function product_by_sub_category($id, $cat_id, Request $request): View
    {
        $categories = Category::latest()->get();
        $brands = Brand::latest()->get();
        $sortOption = $request->input('sort', Session::get('sort', ''));
        ...
        $query = Product::where(['sub_cat_id' => $id, 'cat_id' => $cat_id]);
    ...
    function product_by_child_category($id, $cat_id, $sub_id, Request $request): View
    {
        $categories = Category::latest()->get();
        $brands = Brand::latest()->get();
        $sortOption = $request->input('sort', Session::get('sort', ''));
        ...
        $query = Product::where(['child_cat_id' => $id, 'sub_cat_id' => $sub_id, 'cat_id' => $cat_id]);
    ...
    function product_by_brand($slug, Request $request): View
    {
        ...
        $query = Product::where('cat_id', $brand->id);
